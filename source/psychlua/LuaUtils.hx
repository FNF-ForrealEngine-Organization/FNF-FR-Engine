...
class LuaUtils
{
	...
	public static function getModSetting(saveTag:String, ?modName:String = null)
	{
		...
		else
		{
			...
			MusicBeatState.getState().addTextToDebug('getModSetting: $path could not be found!', FlxColor.RED);
			#else
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		MusicBeatState.getState().addTextToDebug('getModSetting: "$saveTag" could not be found inside $modName\'s settings!', FlxColor.RED);
		#else
